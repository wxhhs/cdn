...
install_depend() {
    ...
    if check_sys sysRelease ubuntu;then
        ...
        if [[ `cat /etc/issue | grep 22.04` != "" ]];then
            apt-get -y install wget python2-minimal cron curl iptables ca-certificates

        else
            apt-get -y install wget python-minimal cron curl iptables ca-certificates

        fi
    elif check_sys sysRelease debian;then
        apt-get update
        apt-get -y install wget python2-minimal cron curl ca-certificates
...
sync_time(){
    ...
    if check_sys sysRelease ubuntu || check_sys sysRelease debian;then
        ...
        ! grep -q "/usr/sbin/ntpdate -u pool.ntp.org" /var/spool/cron/crontabs/root > /dev/null 2>&1 && echo '*/10 * * * * /usr/sbin/ntpdate -u pool.ntp.org > /dev/null 2>&1 || (date_str=`curl update.lotcdn.com/common/datetime` && timedatectl set-ntp false && echo $date_str && timedatectl set-time "$date_str" )'  >> /var/spool/cron/crontabs/root
        service cron restart || systemctl restart cron || true
    elif check_sys sysRelease centos; then
        yum -y install ntpdate wget
        /usr/sbin/ntpdate -u pool.ntp.org || true
	touch /var/spool/cron/root
        ! grep -q "/usr/sbin/ntpdate -u pool.ntp.org" /var/spool/cron/root > /dev/null 2>&1 && echo '*/10 * * * * /usr/sbin/ntpdate -u pool.ntp.org > /dev/null 2>&1 || (date_str=`curl update.lotcdn.com/common/datetime` && timedatectl set-ntp false && echo $date_str && timedatectl set-time "$date_str" )' >> /var/spool/cron/root
...
# 解析命令行参数
TEMP=`getopt -q -o h --long help,master-ver:,agent-ver:,master-ip:,cc-img-url:,master-host:,es-ip:,es-pwd:,ignore-ntp -- "$@"`
...
if [[ $MASTER_VER == "" ]]; then
    ...
    agent_ver=$(curl -v -m 5 "https://raw.githubusercontent.com/wxhhs/cdn/main/upgrades-50800.json" || curl -v -m 5 "https://raw.githubusercontent.com/wxhhs/cdn/main/upgrades-50800.json")
    if [[ "$agent_ver" == "" ]]; then
        echo "无法获取agent版本"
        exit 1
    fi

    agent_ver=`echo $agent_ver  | grep -oP '"agent_ver":"[^"]+' | grep -oP '\d+'`
